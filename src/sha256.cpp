...
Sha256::Sha256(vector<uint8_t> data) {
...
Sha256::Sha256(array<uint8_t, SHA256_BLOCK_SIZE> data) {
    uint64_t L = data.size();
    uint64_t K = 0;
    while( (L + 1 + K + 8) % 64 != 0)
        K = K + 1;
...
array<uint8_t, SHA256_BLOCK_SIZE> Sha256::hash() {
	array<uint8_t, SHA256_BLOCK_SIZE> result;
